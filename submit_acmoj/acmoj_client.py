...
def main():
    ...
    # Source file submission sub-command
    submit_file_parser = subparsers.add_parser("submit-file", help="Submit source file content (cpp)")
    submit_file_parser.add_argument("--problem-id", type=int, required=True, help="Problem ID")
    submit_file_parser.add_argument("--file", type=str, default="src.hpp", help="Path to source file (default: src.hpp)")
    ...
    if args.command == "submit":
        ...
        try:
            ...
            print(f"Error: Failed to read source file '{args.file}': {e}")
            return
        result = client.submit_code(args.problem_id, code_text, language="cpp")
